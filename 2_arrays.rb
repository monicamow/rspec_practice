...
def average(numbers)
  ...
  if numbers.any? { |element| element.class == String }
    average(no_strings(numbers))
  else
    sum = 0
    numbers.each do |n|
      sum += n
...
def no_strings(numbers)
  numbers_without_strings = []
  numbers.map do |str| 
    if str.is_a? String
      numbers_without_strings << str.to_i
    else
      numbers_without_strings << str
    end
  end
  numbers = numbers_without_strings
end
